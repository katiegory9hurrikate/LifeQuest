...
class UserQueries:
            ...
            with pool.connection() as conn:
                with conn.cursor(row_factory=class_row(UserWithPw)) as cur:
                    cur.execute(
                        """
                            SELECT
                                id, username, password, first_name, email
                            FROM users
                            WHERE username = %s
                            """,
                        [username],
    ...
    def get_user_by_id(self, id: int) -> Optional[UserWithPw]:
        """
    ...
    def create_user(
        self, username: str, hashed_password: str, first_name: str, email: str
    ) -> UserWithPw:
        """
            ...
            with pool.connection() as conn:
                with conn.cursor(row_factory=class_row(UserWithPw)) as cur:
                    cur.execute(
                        ...
                        RETURNING *;
                        """,
                        [username, hashed_password, first_name, email],
                    )
                    user = cur.fetchone()
                    if not user:
